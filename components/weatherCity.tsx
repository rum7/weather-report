    ...
    <div className="px-3 pt-3 relative">
        ...
        dataCity.map((wd) => (
          <div key={wd.id} className="flex flex-row flex-wrap items-start">
            <p className={`${fadeData.fade} w-full cn-semibo text-4xl text-king-blue`}>{wd.name}, {wd.sys.country}</p>
            <div className={`${fadeData.fade} flex justify-between w-full items-start`}>
              <div className='flex flex-col md:flex-row mt-3 gap-x-6'>
                <p className='text-6xl mb-2'>{getCurrentTime(wd.timezone)}</p>
              ...
              <div className='flex flex-wrap justify-end mt-3 gap-x-6'>
                ...
                <p className='relative w-[60px] h-[60px] md:w-[60px] md:h-[60px]'>
                  <Image 
